...
EMTcell = data.frame(cellname = names(list_AllCell), Plasticity_v01 = NA, CL_GrowthRate = NA,
                     meanEccentricity = NA, meanCompactness = NA, Displacement_Distance = NA,
                     AvgSpeed = NA, corr_Ecc_SpeedPf_Spearman = NA)
...
for (i in 1:length(list_AllCell)) {
  ...
  meanEccentricity = mean(df$m.eccentricity)
  meanCompactness = mean(df$Compactness)
  ...
  emtval = meanEccentricity * meanCompactness * Displacement_Distance * AvgSpeed * SpCor_Ecc_Speed * GR
  
  # Save the output of the equation and all the varibles used to calculate the Plasticity value
  EMTcell$cellname[i] = cellname
  EMTcell$Plasticity_v01[i] = emtval
  EMTcell$CL_GrowthRate[i] = GR
  EMTcell$meanEccentricity[i] = meanEccentricity
  EMTcell$meanCompactness[i] = meanCompactness
...
colnames(PlasticityCutoff) = c("Plasticity_Score_v01")
...
## mesenchymal cells cutoff: top and bottom 20 % cells
