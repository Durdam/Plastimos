...
EMTcell = cbind(data.frame(do.call('rbind', strsplit(as.character(EMTcell$cellname),'_',fixed=TRUE))), EMTcell[,2:11])
EMTcell$Cell_Line = paste0(EMTcell$X3,"_",EMTcell$X4)
EMTcell$RepObj = paste0(EMTcell$X1,"_",EMTcell$X2)
EMTcell = EMTcell[,-c(1:4)]
...
ix3 = ix2[!is.na(ix2)]
if(all(EMTcell$CL_RepObj[ix1] == dfAll$CL_RepObj[ix3])) print("Perfect match in same order")
...
ix3 = ix2[!is.na(ix2)]
if(all(dfAll$CL_RepObj[ix1] == EMTcell$CL_RepObj[ix3])) print("Perfect match in same order")
