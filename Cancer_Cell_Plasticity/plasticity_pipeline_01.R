...
for (i in 1:nrep_Cell_Line_x_ctrl) {
  ...
  for (j in 1:nobj) {
    ...
    # Compactness: (2D only) Calculated as 4*π*Area/Perimeter2. Equals 1 for a perfectly circular object. For irregular shapes value > 1
    df$Compactness = (df$s.perimeter)^2/(4*pi*df$s.area)
